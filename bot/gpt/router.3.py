...
async def preprocess_message(message: Message) -> list:
    """Preprocess the message immediately upon receipt."""
    content = []
    user_id = message.from_user.id
    token = await tokenizeService.get_token(user_id)
    voice_service = VoiceService(token)

    iso_timestamp = message.date.isoformat()
    header = f"Message at {iso_timestamp}, from: @{message.from_user.username} ({message.from_user.full_name})"
    if message.forward_date:
        forward_info = (
            f"Forwarded from: {message.forward_from.username} ({message.forward_from.full_name})" if message.forward_from
            else f"Forwarded from chat: @{message.forward_from_chat.username} ({message.forward_from_chat.title})"
        )
        header = f"{header}\n{forward_info}"

    if message.reply_to_message:
        reply_content = await preprocess_message(message.reply_to_message)
        reply_text = next((item["text"] for item in reply_content if item["type"] == "text"), "")
        header = f"{header}\nReplied to:\n{quote_message(reply_text)}"
    ...
    if message.photo:
        try:
            photos = [message.photo[-1]] if not hasattr(message, 'album') else [item.photo[-1] for item in message.album]
            photo_links = await get_photos_links(message, photos)
            ...
            if photo_answer:
                ...
                logging.error(f"Failed to preprocess photo, actual value: {photo_answer}")
        except Exception as e:
            content.append({"type": "text", "text": f"{header}\n\nError: unable to transcribe photo"})
            logging.error(f"Failed to preprocess photo: {e}")

    if message.voice or message.audio:
        try:
            messageData = message.voice or message.audio
            file = await message.bot.get_file(messageData.file_id)
            ...
            if voice_response.get("success"):
                ...
                logging.error(f"Failed to preprocess voice/audio: {voice_response.get('text')}")
        except Exception as e:
            content.append({"type": "text", "text": f"{header}\n\nError: unable to transcribe voice/audio: {e}"})
            logging.error(f"Failed to preprocess voice/audio: {e}")

    if message.video:
        content.append({"type": "text", "text": f"{header}\n\nAttached video: {message.video.file_name} Transcription not supported yet."})

    if message.document:
        try:
            doc_text = await transcribe_document(message.document, message.bot)
            caption = message.caption or "No caption"
            content.append({"type": "text", "text": f"{header}\n\nAttached document: {message.document.file_name}\nCaption: {caption}\nContent: {doc_text}"})
        except Exception as e:
            content.append({"type": "text", "text": f"{header}\n\nError: unable to transcribe attached document: {e}"})
            logging.error(f"Failed to preprocess document: {e}")

    return content

async def produce_message(message: Message):
    """Producer: Preprocess and add message content to queue."""
    user_id = message.from_user.id
    ...
    async with lock:
        ...
        preprocessed_content = await preprocess_message(message)
        await queues[key].put(preprocessed_content)
        last_message_times[key] = asyncio.get_event_loop().time()
        logging.info(f"Preprocessed message queued - Key: {key}, Queue size: {queues[key].qsize()}")

async def consumer_task():
    """Consumer: Process queued preprocessed messages after timeout."""
    while True:
            ...
            if current_time - last_time >= timeout and key in queues:
                ...
                async with lock:
                    ...
                    while not queues[key].empty():
                        content = await queues[key].get()
                        contents.append(content)
                        logging.info(f"Dequeued preprocessed content - User: {user_id}")
                    if not contents:
                        continue
                    if queues[key].empty():
                ...
                await handle_messages_with_typing(contents, user_id, chat_id)
...
async def handle_messages_with_typing(contents: list[list], user_id: int, chat_id: int):
    bot = Bot(token=TOKEN)
    async def awaited_operation():
        return await handle_messages(contents, user_id, chat_id, bot)
    await run_with_typing(bot, chat_id, awaited_operation())

async def handle_messages(contents: list[list], user_id: int, chat_id: int, bot: Bot):
    """Handle batched preprocessed contents."""
    last_content = contents[-1]
    last_message = Message(chat=types.Chat(id=chat_id), bot=bot)  # Minimal message object for compatibility
    
    logging.info(f"Handling {len(contents)} preprocessed contents for User: {user_id}")
    
    message_loading = await bot.send_message(chat_id, "**⌛️Ожидайте ответ...**")
...
""")
        ...
        if final_content:
            ...
            if result:
                ...
                token_message = await bot.send_message(chat_id, tokens_message_text)
                if chat_id < 0:  # Group or supergroup
                    await asyncio.sleep(2)
                    await token_message.delete()
            else:
                await message_loading.delete()
        else:
            await bot.send_message(chat_id, "No valid content to process.")
            await message_loading.delete()
    
    except Exception as e:
        await message_loading.delete()
        await bot.send_message(chat_id, DEFAULT_ERROR_MESSAGE)
